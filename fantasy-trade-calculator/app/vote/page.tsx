...
export default async function VotePage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-4 py-8">
      ...
      {!pair ? (
        <p className="text-sm text-zinc-500">Need at least 2 players with values to vote. Add some from the Admin Players page.</p>
      ) : (
        <form className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {pair.map((player) => (
            <button key={player.id} formAction={submitVoteAction.bind(null, pair[0].id, pair[1].id, player.id)}
