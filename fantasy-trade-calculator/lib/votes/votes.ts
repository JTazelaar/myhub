...
export type VotablePlayer = { id: number; name: string; position: string; team: string | null; value: number };
export async function getRandomPlayerPair(): Promise<[VotablePlayer, VotablePlayer] | null> {
  ...
  const eligible = players.filter((p) => p.isActive && valuesMap.has(p.id)).map((p) => ({ ...p, value: valuesMap.get(p.id)! }));
...
export async function recordVote(playerAId: number, playerBId: number, winnerId: number) {
  return prisma.matchupVote.create({ data: { playerAId, playerBId, winnerId } });
}
